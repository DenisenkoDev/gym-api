...
func (urc *UserRoleCreate) SetUserRole(ur userrole.UserRole) *UserRoleCreate {
	urc.mutation.SetUserRole(ur)
	return urc
}
...
func (urc *UserRoleCreate) SetIsClose(b bool) *UserRoleCreate {
	urc.mutation.SetIsClose(b)
	return urc
}

// SetNillableIsClose sets the "is_close" field if the given value is not nil.
func (urc *UserRoleCreate) SetNillableIsClose(b *bool) *UserRoleCreate {
	if b != nil {
		urc.SetIsClose(*b)
	}
	return urc
...
func (urc *UserRoleCreate) SetGymID(id int) *UserRoleCreate {
	urc.mutation.SetGymID(id)
	return urc
}

// SetGym sets the "gym" edge to the Gym entity.
func (urc *UserRoleCreate) SetGym(g *Gym) *UserRoleCreate {
	return urc.SetGymID(g.ID)
}
...
func (urc *UserRoleCreate) defaults() {
	...
	if _, ok := urc.mutation.IsClose(); !ok {
		v := userrole.DefaultIsClose
		urc.mutation.SetIsClose(v)
	}
}
...
func (urc *UserRoleCreate) check() error {
	...
	if _, ok := urc.mutation.IsClose(); !ok {
		return &ValidationError{Name: "is_close", err: errors.New(`ent: missing required field "UserRole.is_close"`)}
	}
	if len(urc.mutation.GymIDs()) == 0 {
		return &ValidationError{Name: "gym", err: errors.New(`ent: missing required edge "UserRole.gym"`)}
	}
...
func (urc *UserRoleCreate) createSpec() (*UserRole, *sqlgraph.CreateSpec) {
	...
	if value, ok := urc.mutation.IsClose(); ok {
		_spec.SetField(userrole.FieldIsClose, field.TypeBool, value)
		_node.IsClose = value
	}
	if nodes := urc.mutation.GymIDs(); len(nodes) > 0 {
