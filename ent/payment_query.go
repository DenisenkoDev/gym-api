...
func (pq *PaymentQuery) QueryAbonement() *AbonementQuery {
	query := (&AbonementClient{config: pq.config}).Query()
...
func (pq *PaymentQuery) Clone() *PaymentQuery {
	...
	return &PaymentQuery{
		...
		withAbonement: pq.withAbonement.Clone(),
...
func (pq *PaymentQuery) WithAbonement(opts ...func(*AbonementQuery)) *PaymentQuery {
	query := (&AbonementClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pq.withAbonement = query
	return pq
}
...
func (pq *PaymentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Payment, error) {
	var (
		...
		_spec       = pq.querySpec()
		loadedTypes = [1]bool{
			pq.withAbonement != nil,
		}
	)
	if pq.withAbonement != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := pq.withAbonement; query != nil {
		if err := pq.loadAbonement(ctx, query, nodes, nil,
			func(n *Payment, e *Abonement) { n.Edges.Abonement = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *PaymentQuery) loadAbonement(ctx context.Context, query *AbonementQuery, nodes []*Payment, init func(*Payment), assign func(*Payment, *Abonement)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Payment)
	for i := range nodes {
		if nodes[i].payment_abonement == nil {
			continue
		}
		fk := *nodes[i].payment_abonement
	...
	query.Where(abonement.IDIn(ids...))
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "payment_abonement" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
