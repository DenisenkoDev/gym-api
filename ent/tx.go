...
func (tx *Tx) init() {
	...
	tx.AbonementType = NewAbonementTypeClient(tx.config)
	tx.Credential = NewCredentialClient(tx.config)
	tx.Gym = NewGymClient(tx.config)
	tx.ManagerRole = NewManagerRoleClient(tx.config)
	tx.Payment = NewPaymentClient(tx.config)
	tx.PaymentRequisite = NewPaymentRequisiteClient(tx.config)
	tx.UsageMode = NewUsageModeClient(tx.config)
