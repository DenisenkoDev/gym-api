...
func (urq *UserRoleQuery) QueryGym() *GymQuery {
	query := (&GymClient{config: urq.config}).Query()
...
func (urq *UserRoleQuery) WithGym(opts ...func(*GymQuery)) *UserRoleQuery {
	query := (&GymClient{config: urq.config}).Query()
...
//		GroupBy(userrole.FieldUserRole).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
func (urq *UserRoleQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*UserRole, error) {
	...
	if urq.withGym != nil || urq.withUser != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := urq.withGym; query != nil {
		if err := urq.loadGym(ctx, query, nodes, nil,
			func(n *UserRole, e *Gym) { n.Edges.Gym = e }); err != nil {
			return nil, err
		}
	}
...
func (urq *UserRoleQuery) loadGym(ctx context.Context, query *GymQuery, nodes []*UserRole, init func(*UserRole), assign func(*UserRole, *Gym)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*UserRole)
	for i := range nodes {
		if nodes[i].user_role_gym == nil {
			continue
		}
		fk := *nodes[i].user_role_gym
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_role_gym" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (urq *UserRoleQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*UserRole, init func(*UserRole), assign func(*UserRole, *User)) error {
	...
	nodeids := make(map[int][]*UserRole)
	for i := range nodes {
		if nodes[i].user_role_user == nil {
			continue
		}
		fk := *nodes[i].user_role_user
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_role_user" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
